...
Object.keys(parsetype).forEach(
  (key) => (parsetype[key].tw = parsetype[key].cn)
);
...
const settingKey = {
  ...
  msg = {
    ...
    normal: {
      ...
      storage_load_fleet(fleet_id, name) {
        return this._t(
          `Fleet ${fleet_id} [${decodeURIComponent(name)}] loaded`
        );
      },
  ...
  app = {
    option: {
      ...
      compactMode(ele, index) {
        ...
        function subClass(class_data, class_str, set_layout, target_key) {
          Object.keys(class_data).forEach((layout) => {
            class_data[layout] = class_data[layout].replace(
              ` ${class_str}`,
              ""
            );
            if (layout == set_layout) {
    ...
    util: {
      ...
      dumpTechData() {
        document
          .querySelectorAll("#tech_data_area_expand input")
          .forEach((e) => {
      ...
      for (let index in shipInApp.item) {
        ...
        if (item.id === "000000") {
          ...
          if (index === "0") {
            ...
            //ship
            ui_table.copy_ship.forEach(
              (key) => (app_item[key] = shipInList[key])
            );
    ...
    async equipDisplay() {
      ...
      async function limitEquip(display_list) {
        ...
        equipOnShip.forEach((id) => {
          ...
          if (!isNaN(limit)) {
            if (limit > 0 && !limit_list.includes(limit))
    ...
    setEquip(item, save = false, skip_level = false) {
      ...
      if (id != 666666) {
        ...
        ui_table.copy_equip.forEach(
          (key) => (itemInApp[key] = itemInList[key])
        );
        // set level
    ...
    async initialize() {
      ...
      if (indexedDB && window.idb) {
        ...
        pos.innerHTML = `
                ...
                `;
        pos.className =
          "h5 text-danger text-center text-monospace mx-auto my-5";
      ...
      function setSlider() {
        ...
        $("#spweaponselect").on("hide.bs.modal", () =>
          app.setLevel("spweapon")
        );
      }
      ...
      async function createAllEquip() {
        ...
        let pos = document.querySelector("#equiplist"),
          html = sortedEquip.map((item) =>
            createNewItem(item, _loading_.equip)
          );
        setTimeout(() => {
          pos.innerHTML = html.join("");
